...
interface ChapterManageResourcesPageProps {
  params: Promise<{ chapter: string }>
}

export default async function ChapterManageResourcesPage({
  params,
}: ChapterManageResourcesPageProps) {
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {total} resource{total !== 1 ? 's' : ''} — {chapterRow.name}
          </p>
        </div>
        <Link
                ...
                <tr key={resource.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      <Link
                        href={`/${chapterSlug}/resources/manage/${resource.id}/edit`}
